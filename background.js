...
const DEFAULT_STATE = {
  ...
  [STORAGE_KEYS.MODE]:       'allowlist',
  [STORAGE_KEYS.ALLOWLIST_RULES]: [
...
async function getSettings() {
  ...
  const mode  = state[STORAGE_KEYS.MODE];
  const rules = mode === 'allowlist'
    ? (state[STORAGE_KEYS.ALLOWLIST_RULES] || [])
    : (state[STORAGE_KEYS.DENYLIST_RULES]  || []);
  return {
 ...
 * @property {'allowed'|'blocked'} status
 * @property {string} reason      - Why blocked/allowed
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === 'CLEAR_LOG') {
    chrome.storage.local.set({ [STORAGE_KEYS.LOG]: [] })
  ...
  if (message.type === 'GET_LOG') {
    getState()
      .then(s => sendResponse({ log: s[STORAGE_KEYS.LOG] }))
      .catch(e => sendResponse({ log: [], error: e.message }));
    return true;
  }
  if (message.type === 'GET_ALL_MIME_TYPES') {
    sendResponse({ mimeTypes: DEFAULT_STATE[STORAGE_KEYS.ALLOWLIST_RULES] });
    return true;
  }
});